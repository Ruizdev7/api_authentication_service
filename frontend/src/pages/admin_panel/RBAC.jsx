...
import { useGetRBACModulesQuery } from "../../redux_app/services/rbacModulesAPI";
import { useUpdatePermissionsMutation } from "../../redux_app/services/rbacAPI";

import { Read, Create, Update, Delete } from "./../../assets/SVG"
...
function RBAC() {
  ...
  const [trigger, { data, error: error_update_permission }] =
    useUpdatePermissionsMutation(ccnRbacModule);
          ...
          >
            {list_roles
              ? list_roles.Role.map((role) => (
                <option key={role.ccn_role} value={role.ccn_role}>
                  {role.role}
                </option>
              ))
              : null}
      ...
      <div className="container m-auto overflow-y-scroll h-[45vh] rounded-lg shadow ">
        <table className="w-full border-gray-400 h-[35px] rounded-lg border-2">
          ...
          <tbody className="divide-y divide-gray-200 ">
            {list_rbac_by_role
              ? list_rbac_by_role.rbacByRole.map((i) => (
                  ...
                  <td className="p-3 text-xs text-left text-gray-700 whitespace-nowrap uppercase font-extrabold">
                    {i.length != 0
                      ? list_rbac_modules.rbac.find(
                        (module) =>
                          module.ccn_rbac_module === i.ccn_rbac_module
                      ).rbac_module
                      : i.ccn_rbac_module}
                  </td>
